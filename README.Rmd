...
Ok, finally we're at the interesting stuff! The task at hand is to quantify where there is a significant difference between the success of plays, as measured by EPA, grouped by two binary conditions:
...
There is evidence for a statistically significant difference for both groups.
...
One can clearly see the gap between the EPA medians when there is non-man coverage on pick plays (upper-left and mid-left facets), which our Frequentist analysis (based on the t-tests) has indicated is statistically significant.
