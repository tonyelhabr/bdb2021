...
-   The insignificant difference for `"EPA | Pass Successful? N & Same Defender? N"` (i.e. given an unsuccessful pass, EPA when the targeted receiver is not covered by their initial defender is not significantly different on pick plays versus all other passes) is best interpreted by comparing it to the significant difference in its direct counterpart, `"EPA | Pass Successful? N & Same Defender? N"`. The latter result implies that non-man coverage has a significant relationship with EPA when the pass is not successful, while the former result implies that man coverage does not. Again, the real-world implication is that non-man coverage is impactful.
