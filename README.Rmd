...
Both when using a causal approach and a more traditional machine learning approach, I find no evidence for targeted pick route combinations having a significant effect on play success. This contrasts with the result found from simply comparing the difference in EPA means with [t-tests](https://en.wikipedia.org/wiki/Student%27s_t-test), i.e. a [Frequentist approach](https://en.wikipedia.org/wiki/Frequentist_inference)). Likewise, when investigating the role the targeted defender in pick plays, I find that the causal and machine learning approaches agree with one another, but disagree with the un-adjusted Frequentist implication that there is a significant difference between a strict, man-to-man coverage and other coverages in reducing play succes.
...
Now we find that there is only 2 (and 1 is barely significant) of 9 t-tests that shows significance. (Previously there were 7 significant results.) Interestingly, the 1 result that is more strongly significant---`"EPA | Pass Successful? Y ~ Same Defender? Y"`---does not agree with any of the Frequentist findings. Here, the implication is that man coverage does worse against pick plays compared to other plays given that the pass is successful.[^7]

[^7]: I'm not sure how much faith I put in this deduction; it could speak to some fault in the matching.
...
![viz_prop_probs_has_same_defender](https://github.com/tonyelhabr/bdb2021-data/raw/master/figs/viz_prop_probs_has_same_defender.png)

Note that the propensity score matching has a different distribution due to the change in response variable. 

![viz_love_has_same_defender](https://github.com/tonyelhabr/bdb2021-data/raw/master/figs/viz_love_has_same_defender.png)
...
![viz_epa_has_same_defender](https://github.com/tonyelhabr/bdb2021-data/raw/master/figs/viz_epa_has_same_defender.png)
...
Lastly, the apples-to-apples t-test table shows lack of significance across nearly all conditions.
...
The robustness of a causal approach that attempts to account for mutliple backdoor pathways given a final response variable that is non-linear (EPA) is a little suspect to me, hence why I now resort to treating this as a "traditional" machine learning problem.[^8]
...
These suggest that whether a play involves a target pick and the type of coverage of the targeted defender are not very important for predicting EPA.
...
So, even if our assessment is that there is no significant relationship (or causal relationship, in the case of our causal analysis) between targeted pick plays and type of coverage on targets, I think these EPA scatters at least tell us that the methodology for identifying picks and coverage is somewhat sound.

## Conclusion

While t-tests show statistically significant magnitudes of EPA for two treatments---(1) target pick plays and (2) coverage on the targeted receiver---adjusting for game situation and player tracking-derived features with a causal approach shows that these differences are not caused by pick routes. Likewise, a more traditional machine learning approach, leveraging an `{xgboost}` model, deduces that there is no significant relationship for the two treatments. Despite the lack of significance at the holistic level, the individual descriptive results (for both offensive and defensive players) derived from identifying pick routes and targeted defensive coverage seemed to agree with anecdotal knowledge.
