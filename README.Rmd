...
*Note to the reader: Although I refer to myself as "we" in much of this notebook, note that I'm just one person and have he/his pronouns. I just happen to like how "we" sounds when writing things out in a pseudo-research setting. I promise I do not have some kind of God complex.*

# Introduction

## Execuive Summary

Using [Frequentist analysis](https://en.wikipedia.org/wiki/Frequentist_inference), we find that "non-man" coverage (anything that is not clearly man coverage, which certainly includes pure zone, but which may also include "matchup" man coverage) is significantly less successful (in terms of EPA) against pick route combinations where one of the involved receivers is targeted, as compared to the same such coverage against plays where the targeted receiver is not involved in a pick combination. Additionally, we find that there is no significant difference in playing man coverage against pick route combinations involving the eventual targeted receiver compared to man coverage in other plays. (This deduction is not necessarily implied by the prior conclusion.)

Using a causal approach, accounting for play context, we come to ...
...
To provide some motivation to the reader to read through this very long (maybe unnecessarily long) notebook, below is the pick play that resulted in the highest expected points added (EPA) of all plays in the 2018 season.
...
3.  Quantify the relative success pf pick route combinations from the offensive persepective, as well as the success of certain defensive coverage against it.

## Actionable Outcomes

While the final evaluation is somewhat wholistic (i.e. it tries to come to a conclusion about covering pick plays in general, based on statistical aggregations across all plays), we believe that there are several actionable learning outcomes:

1.  Teams could use the methodology for identifying pick route combinations for the purpose of offensive play classification (i.e. "Did the offense run a pick route?"), likely coming in the form of tagging of video analysis. Perhaps teams already do this or have a third-party do this for them, but if not, it could certainly be worthwhile for providing an clear, reproducible classification of pick plays, not subjected to the biases of human video reviewers.

2.  Teams could use this analysis to supplement their evaluation of defensive schemes when covering pick route combinations, perhaps on a weekly basis (preparing for the next week's game). Likewise, they may choose to run more or less pick route combinations on offense depending on their evaluation of their opponent's ability to cover them. In particular, they could evaluate the skill of specific defenders in covering such plays. Of course, we cannot know for certain whether individual defenders were subjected to "non-optimal" defensive play calls in certain situation since we do not know the team defensive coverages for all plays (and also how teams have coached defenders to adapt in response to certain offensive actions), so this analysis is limited in that sense.
