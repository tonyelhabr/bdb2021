...
I use an `{xgboost}` model that mirrors the `{nflfastR}` specification, adding in our terms for the two exposures and the tracking control features discussed before. [SHAP values](https://christophm.github.io/interpretable-ml-book/shapley.html) tell us how each feature in this model contributes to predictions (both direction and magnitude).
