...
viz_defenders_intersect_adj_by_coverage <-
  defenders_intersect_adj_by_coverage %>% 
  mutate(
    across(c(has_same_defender), .binary_factor_to_lgl),
    across(
...
viz_defenders_intersect_adj_by_coverage
do_save_plot(viz_defenders_intersect_adj_by_coverage)
...
#   ) %>%
#   ungroup() %>% 
#   filter(is_target_picked == 1L) %>% 
#   select(-is_target_picked)
# nonpick_play_meta_viz_init
...
# nonpick_play_meta_viz <-
#   nonpick_play_meta_viz_init %>% 
#   inner_join(plays_w_pick_def_info) %>% 
#   rename(sec_intersec = sec) %>% 
#   .filter_plays()
...
pick_plays_meta_viz_init <-
  ...
  mutate(across(c(is_pass_successful, has_same_defender), .binary_factor_to_lgl)) %>% 
  filter(is_target) %>%
  .filter_plays()
pick_plays_meta_viz_init
