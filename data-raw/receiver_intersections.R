...
identify_intersections_until <- function(data, sec = 0.5, .verbose = bdb2021:::.get_verbose()) {
  ...
  eligible_grps <-
    data %>%
    dplyr::filter(.data$n_route >= 2L)

  ineligible_grps <-
    data %>%
    dplyr::anti_join(
      eligible_grps %>%
        dplyr::distinct(.data$game_id, .data$play_id, .data$y_side_init, .data$n_route, .data$nfl_id, .data$frame_id),
      by = c('game_id', 'play_id', 'y_side_init', 'n_route', 'nfl_id', 'frame_id')
    )
  ...
  intersections_init <-
    ...
    dplyr::select(.data$game_id, .data$play_id, .data$y_side_init, .data$n_route, .data$nfl_id, .data$frame_id, .data$x, .data$y) %>%
  ...
  intersections <-
    ...
    dplyr::anti_join(
      bad_intersections,
      by = c('game_id', 'play_id', 'y_side_init', 'n_route', 'data')
    ) %>%
    dplyr::mutate(
  ...
  res <-
    ...
    dplyr::arrange(.data$game_id, .data$play_id, .data$y_side_init)
  res
  }
  ...
  receivers_at_seconds <-
    ...
    dplyr::inner_join(
      seconds_frames %>%
        ...
        dplyr::group_by(.data$game_id, .data$play_id, .data$y_side, .data$x_side, .data$frame_id) %>%
        dplyr::mutate(n_route = dplyr::n()) %>%
        dplyr::ungroup() %>%
        dplyr::select(.data$game_id, .data$play_id, .data$nfl_id, .data$x_side, .data$y_side, .data$n_route),
      by = c('game_id', 'play_id', 'nfl_id')
    ) %>%
    dplyr::rename(x_side_init = .data$x_side, y_side_init = .data$y_side)
